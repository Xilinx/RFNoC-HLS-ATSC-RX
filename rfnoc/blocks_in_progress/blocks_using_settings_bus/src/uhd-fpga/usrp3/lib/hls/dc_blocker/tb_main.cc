...
int main()
{
    ...
    if (LONG_FORM)
    	file_o = fopen("tb_out_128_long", "rb");
    else
    	file_o = fopen("tb_out_128_short", "rb");
	...
	dc_blocker(0, 0, D, 128, true);
	dc_blocker(0, 0, LONG_FORM, 129, true);
	...
	for(int i = 0; i < ITER; i++) {
		...
		dc_blocker(input, output, 0, 0, false);
